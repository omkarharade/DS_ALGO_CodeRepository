...
void update(vector<int>& segTree, vector<int>& lis, int s, int e, int tIdx, int idx , int val) {
	...
	if (idx > mid) {
		...
		update(segTree, lis, mid + 1, e, 2 * tIdx + 1, idx, val);
	}
	else {

		update(segTree, lis, s, mid, 2 * tIdx, idx, val);
	}
...
int query(vector<int>& segTree, int s, int e, int tIdx, int left, int right) {
	...
	int leftMax = query(segTree, s, mid, 2 * tIdx, left, right);
	int rightMax = query(segTree, mid + 1, e, 2 * tIdx + 1, left, right);
...
void solve() {
	...
	for (int i = 0; i < n; ++i)
	{
		int element = arr[i].ff;
		int index = arr[i].ss;

		int prevLIS = query(segTree, 0, n - 1, 1, 0, index - 1); // get prev computed LIS
		update(segTree, lis, 0, n - 1, 1, index, prevLIS + 1); // and add 1
	...
	for (int i = 0; i < n; ++i)
	{
		max_subarray_sum = max(max_subarray_sum, lis[i]);
	}

	cout << max_subarray_sum << nline;
